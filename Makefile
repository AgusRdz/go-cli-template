...
build:
	docker compose run --rm dev go build -ldflags="$(LDFLAGS)" -o bin/mytool .
...
BINARY := bin/mytool$(EXT)

ifeq ($(GOOS),windows)
  INSTALL_DIR ?= $(LOCALAPPDATA)/Programs/mytool
else
...
install:
	...
	@mkdir -p "$(INSTALL_DIR)"
	cp $(BINARY) "$(INSTALL_DIR)/mytool$(EXT)"
	@echo "installed mytool $(VERSION) ($(GOOS)/$(GOARCH)) to $(INSTALL_DIR)/mytool$(EXT)"
...
cross:
	docker compose run --rm dev sh -c "\
		CGO_ENABLED=0 GOOS=linux   GOARCH=amd64 go build -ldflags='$(LDFLAGS)' -o bin/mytool-linux-amd64 . && \
		CGO_ENABLED=0 GOOS=linux   GOARCH=arm64 go build -ldflags='$(LDFLAGS)' -o bin/mytool-linux-arm64 . && \
		CGO_ENABLED=0 GOOS=darwin  GOARCH=amd64 go build -ldflags='$(LDFLAGS)' -o bin/mytool-darwin-amd64 . && \
		CGO_ENABLED=0 GOOS=darwin  GOARCH=arm64 go build -ldflags='$(LDFLAGS)' -o bin/mytool-darwin-arm64 . && \
		CGO_ENABLED=0 GOOS=windows GOARCH=amd64 go build -ldflags='$(LDFLAGS)' -o bin/mytool-windows-amd64.exe ."

...
// TODO: update repo to match your GitHub repository (owner/repo).
const repo = "agusrdz/mytool"
...
// TODO: replace with your actual public key before publishing.
// Generate with: openssl genpkey -algorithm ed25519 -out private.pem
//               openssl pkey -in private.pem -pubout -out public_key.pem
//               openssl pkey -in private.pem -noout -text (copy the pub key hex, 32 bytes = 64 hex chars)
// Store the base64-encoded private key as SIGNING_KEY in GitHub Actions secrets.
...
func Run(currentVersion string) {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "mytool: failed to check for updates: %v\n", err)
		os.Exit(1)
	}
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "mytool: failed to find current binary: %v\n", err)
		os.Exit(1)
	}

	tmpPath := exe + ".tmp"
	if err := download(url, tmpPath); err != nil {
		fmt.Fprintf(os.Stderr, "mytool: update failed: %v\n", err)
		os.Remove(tmpPath)
		os.Exit(1)
	}
	...
	if err := verifyChecksum(tmpPath, latest, binaryName); err != nil {
		fmt.Fprintf(os.Stderr, "mytool: verification failed: %v\n", err)
		os.Remove(tmpPath)
		os.Exit(1)
	}

	if err := replaceBinary(exe, tmpPath); err != nil {
		fmt.Fprintf(os.Stderr, "mytool: failed to replace binary: %v\n", err)
		os.Remove(tmpPath)
		os.Exit(1)
	}
...
func buildBinaryName() string {
	...
	return fmt.Sprintf("mytool-%s-%s%s", goos, goarch, ext)
}
...
// signature using the embedded public key, then verifies the SHA256 of the binary.

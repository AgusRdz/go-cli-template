...
func disabledFlagPath() string {
	home, _ := os.UserHomeDir()
	return filepath.Join(home, ".config", "mytool", ".disabled")
}
...
func Install(version string) {
	exe, err := exePath()
	if err != nil {
		fmt.Fprintf(os.Stderr, "mytool: failed to get executable path: %v\n", err)
		os.Exit(1)
	}
	...
	if err := saveSettings(settings); err != nil {
		fmt.Fprintf(os.Stderr, "mytool: failed to write settings: %v\n", err)
		os.Exit(1)
	}

	fmt.Printf("mytool %s hook installed\n", version)
	fmt.Printf("  binary: %s\n", exe)
	fmt.Printf("  config: %s\n", claudeSettingsPath())
}

...
func Path() string {
	...
	return filepath.Join(home, ".config", "mytool", "config.yml")
}
...
func Load(projectDir string) (*Config, error) {
	...
	// Load project config (.mytool.yml), walk up from projectDir
	if projectDir != "" {
		dir := projectDir
		for {
			projectPath := filepath.Join(dir, ".mytool.yml")
...
func Show(projectDir string) {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "mytool: failed to load config: %v\n", err)
		return
	}
	fmt.Printf("config: %s\n\n", Path())
